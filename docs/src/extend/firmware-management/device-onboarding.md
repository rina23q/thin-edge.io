...
2. A valid ssh key which matches the authorized key loaded in the built image
    ...
    :::info
    If you are using an image built using **Rugpi**, the authorized ssh key/s should have been included in the image. If you can't ssh into your device please review the [build instructions](./building-image/rugpi/tutorial.md#clone-project) again to make sure the correct ssh key/s was added during build time.
    :::
