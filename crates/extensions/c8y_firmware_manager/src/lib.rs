...
pub struct FirmwareManagerBuilder {
    ...
    timer_sender: Option<DynSender<SetTimeout<TimerKey>>>,
...
impl ServiceConsumer<SetTimeout<TimerKey>, Timeout<TimerKey>, NoConfig> for FirmwareManagerBuilder {
    fn get_config(&self) -> NoConfig {
        NoConfig
    }

    fn set_request_sender(&mut self, sender: DynSender<SetTimeout<TimerKey>>) {
        self.timer_sender = Some(sender);
    }

    fn get_response_sender(&self) -> DynSender<Timeout<TimerKey>> {
